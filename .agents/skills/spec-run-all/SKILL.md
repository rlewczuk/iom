...
4. Pass each child the exact `repo_root`, prepared paths and task identity, integration branch/head, exclusive task scope, and known cross-task interface contracts. The child must use assigned-worktree mode below. It must not delegate, implement nested descendants, integrate, or perform validation.
5. Record every preparation failure in `outcomes` as `blocked`, including failures where no worktree could be prepared. Wait until every dispatched child has ended. Record child runtime failures as `failed`, external prerequisites as `blocked`, and provisional successes as `ready` only when no same-wave preparation refresh is needed. Retain concrete reasons and returned worktree/commit evidence. Independent tasks remain eligible in later queue calls.
...
The child receives the exact `repo_root`, `worktree`, `task_path`, `spec_path`, `annotation_path`, feature branch, integration branch/head, and exclusive task scope. It reads `skill://spec-run-task` explicitly and follows section 4's assigned-worktree child mode. Invoke the existing helper with `--repo '<repo_root>'` from the assigned worktree; never reinterpret that worktree as the integration checkout:
...
- never use direct Git mutation, manually edit `task.md`, integrate, rebase, or delegate/nested-run another task;
