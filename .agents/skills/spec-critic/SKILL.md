...
Before this workflow, the root MUST read `skill://boss` and follow Boss's canonical dispatch, brief, and verification rules. This skill adds only spec-critic routing and does not import any C++ review protocol. The root supervisor MUST already resolve to `@slow`; the skill MUST NOT switch models, edit model configuration, silently fall back, or inherit an expensive parent when a requested lane is unavailable. Before any dispatch, the root checks the effective `modelRoles`, `task.agentModelOverrides`, and `task.agentAdvisor` values and verifies that the `@slow`, `@task`, `@smol`, and `@advisor` aliases resolve to their intended models, with no unintended passive advisor on cheap workers. A conflict or unavailable role is reported and dispatch stops.
