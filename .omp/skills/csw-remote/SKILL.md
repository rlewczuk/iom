...
```bash
.omp/csw/bin/csw-remote-exec rocm task-123 'cmake -S . -B build && cmake --build build -j'
.omp/csw/bin/csw-remote-exec rocm task-123 'ctest --test-dir build --output-on-failure'
```
