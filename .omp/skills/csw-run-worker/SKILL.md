...
Each stage is once per leaf across all retries, including later verifier repairs, not once per handoff or failing symptom. Retain both reports and subsequent attempts; do not reset the stages, start another rescue identity or recurse from Yoda. A failed implementation report requires both stages and owner follow-through to have left a real unresolved failure. `RESCUE` records `csw-debug` and `csw-yodacoder` separately: not needed with reason, or actual agent/job ID, report artifact, selected mode, proposal/change disposition, post-rescue commands/results and cleanup state. Preserve those facts through helper Summary/Verification/Errors even when rescue succeeds.
...
For `csw-run`, reuse the control peer's successful preflight record forwarded by the root. Otherwise run `.omp/csw/bin/csw_preflight --repo <repo> --workflow csw-run-worker --pretty` once before implementation or container dispatch. It validates both rescue profiles and aliases, including `@csw-yoda`. A failed preflight is retained failure evidence; never substitute another profile/model.
...
- Backend-specific code: configure/build the affected targets, run relevant unit/regression tests, and run the backend conformance suite on **each touched backend** using its configured remote host where required. A CPU pass does not validate CUDA, ROCm, or SYCL code.
