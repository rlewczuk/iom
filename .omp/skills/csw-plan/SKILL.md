...
For example, when a shared API implementation is genuinely missing, use `shared-api → {CUDA, ROCm, SYCL}` and, only if required, `{CUDA, ROCm, SYCL} → cross-backend-verification`. Never use `CUDA → ROCm → SYCL` merely as a preferred implementation order. With the API already available, all three backend tasks have empty blocker lists.
