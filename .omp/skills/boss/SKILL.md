...
For a review, the root/supervisor must already run on `@slow`; configuring `modelRoles.slow` or the root model is an operator concern, not a hidden fallback in this skill. The root owns scope resolution, reviewed-state identity, specification mapping, routing, candidate acceptance, the final assignment table, task materialization decisions, and all final verification.
