...
Preserve the complete JSON result as the invocation's environment evidence. This executable is the sole source for the Git baseline, effective OMP settings and role resolution, model availability, and bundled Boss-agent profile contracts. Exit status `0` means `ok: true`; exit status `2` or `ok: false` is a concrete blocker to Boss dispatch. Report its `errors` and stop before dispatch; do not bypass it, inspect OMP configuration files or databases, invoke `omp config`/`omp models`, read agent frontmatter for discovery, or reconstruct Git state with ad-hoc commands. A skill cannot switch the already-running root model. Determine the running root's launch alias and model identity only from session identity supplied by OMP and the preflight result; do not perform additional environment discovery. The automatically approved root launch aliases are `@slow` and `@csw-yoda`.

After a successful preflight, continue without prompting when the running root was launched with `@slow` or `@csw-yoda`, or when its model identity matches the model resolved for `@slow`. Any other root-model mismatch is a consent gate, not a preflight error or an unconditional blocker. Before any further repository inspection or dispatch, display a warning that names the running root alias/model, the automatically approved aliases, and the model resolved for `@slow`; explain that continuing may reduce review quality or invalidate the workflow's root-model assumptions; and ask the user: `Proceed with Boss using <running-model>?` Offer explicit `Proceed` and `Stop` choices and wait for the answer. If the user proceeds, record the accepted mismatch as an invocation deviation and continue with the current root model; if the user stops, end the workflow. Ask only once per invocation. Throughout Boss and its dependent workflows, references to a root `@slow` session describe the preferred deployment and include either automatic `@csw-yoda` approval or a current-model deviation explicitly accepted through this gate.
...
For a review, the preferred deployment runs the root/supervisor on `@slow`; `@csw-yoda` is also automatically approved, while any other running model may proceed only after the user accepts the mismatch through the deterministic preflight consent gate. Configuring model roles or the root model remains an operator concern, not a hidden fallback in this skill. The root owns scope resolution, reviewed-state identity, specification mapping, routing, candidate acceptance, the final assignment table, task materialization decisions, and all final verification.
