---
description: Split .cswd/tasks/<task-name>[/subdirectory]/spec.md into parallel-ready typed tasks; optional --impl or --hld
---
...
**Mandatory dependency policy:** maximize safe parallelism across all meaningful outcomes, including independent tensor operations and backend implementations. Start with empty blocker lists; add an edge only for a proven required producer output, with non-empty output-specific `remarks`. Shared interfaces/files, implementation analogues, numbering, priority, planning phases, and verification/integration resources do not justify edges on their own. Use genuine common prerequisites with independent consumers and only required joins, never arbitrary predecessor chains. Before writing, apply the skill's dependency-minimization gate, including indirect-path checks; expose the resulting parallel branches in `summary.md`. `task_ctl plan` orders the supplied graph but cannot judge whether its dependencies are necessary.
