...
- For accelerator checks, read `skill://csw-remote`; use the integration-checkout sync/exec helpers with the exact worktree, required evidence environment paths, and a unique mirror. Follow sync-before-every-exec, finite local/remote deadlines, bounded GPU locks, SYCL setup, TTNN and cleanup rules. Only helper `checkpoint` may create temporary history when the worker requires it to sync new source/tests; report any checkpoint to the owner for final consolidation.
