...
public:
    ...
    public:
        ...
        [[nodiscard]] hipDeviceptr_t staging() const noexcept { return staging_; }
    ...
    private:
        Lease(
                ...
                hipDeviceptr_t staging, std::size_t capacity)
                : pool_(&pool), index_(index), staging_(staging),
                  capacity_(capacity) {}

        StagingSlotPool* pool_;
        std::size_t index_;
        hipDeviceptr_t staging_;
...
private:
    struct Slot {
        hipDeviceptr_t staging = nullptr;
        std::size_t capacity = 0;
        bool active = false;
    };

    [[nodiscard]] hipDeviceptr_t allocate_locked(std::size_t bytes);
